...
local Builder,parent = torch.class('ULGBuilder','DataBuilder')

function Builder:__init(subjects, imageWidth, imageHeight, imageCenterType, dynamicExtraction, extractedFaceWidth)
    parent.__init(self,paths.concat(paths.dirname(paths.thisfile()), 'kinect'), 'ULG', subjects, imageWidth,imageHeight,imageCenterType, dynamicExtraction, extractedFaceWidth)
end
...
function Builder:listFilesForSubject(main_folder,subject)
    ...
    local function valid(f)
        if f:match('%d.png') then return true
        else return false
        end
    end
    local filenames = {}
    for f in paths.files(paths.concat(main_folder,subject,'nir/'),valid) do
        filenames = array_concat(filenames, {paths.basename(f,'.png')})
    end
    return filenames
end
local points_s = -1
...
function Builder:loadLandmarks(main_folder,subject, filename)
    if subject ~= points_s then
        points_s = subject
        require 'csvigo'
        local filename = paths.concat(main_folder, subject,'annot-s2.txt')
        points_data = csvigo.load{path=filename,separator=' ',mode='raw', header=false, verbose=false}
        --print(points_data)
    end
    local points = points_data[filename+1]

    -- format to [x1 x2 x3 ... y1 y2 y3 ...]
    points = torch.Tensor(points):view(-1,2):t():contiguous():view(-1)
    return points:add(1)
end
function Builder:loadImage(main_folder,subject,filename)
    return image.load(paths.concat(main_folder,subject,'nir', filename..'.png'),1,'float'):squeeze()
end
